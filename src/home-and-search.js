...
// @match        https://anitaku.to/popular.html*
// @match        https://anitaku.to/search.html*
...
`;
  ...
  var c = [];
  if (location.href == "https://anitaku.to/popular.html")
    animes.forEach((x) => {
      c.push({
        name: x.name,
        url:
  ...
  search.onkeydown = () => {
    if (event.keyCode == 13) {
      location.href =
        "https://anitaku.to/search.html?keyword=" + search.value;
    }
  };
  ...
  if (
    ...
    prv.onclick = () => {
      ...
      location.href = "https://anitaku.to/popular.html?page=" + (num - 1);
    };
    nav.appendChild(prv);
  }
